                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full bg-gray-900/50 rounded-lg overflow-hidden">
                        ...
                        <tbody className="divide-y divide-gray-700">
                            {chartData.datasets.flatMap((dataset, datasetIndex) => {
                                const vehiculeNom = dataset.label || '';
                                return chartData.labels?.map((week, weekIndex) => {
                                    const consommation = dataset.data[weekIndex];
                                    if (consommation === 0) return null;

                                    // Trouver les données originales pour cette semaine et ce véhicule
                                    const originalData = chartData.originalData?.find(
                                        (d: WeekData) => d.week === week && d.vehicule_nom === vehiculeNom
                                    );
                                    ...
                                    return (
                                        <tr key={`${vehiculeNom}-${week}`} className="hover:bg-gray-700/50 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-sm text-gray-300 text-right">
                                                {originalData?.litres || 'N/A'} L
                                            </td>
                                            <td className="px-4 py-3 text-sm text-gray-300 text-right">
                                                {originalData?.km || 'N/A'} km
                                            </td>
